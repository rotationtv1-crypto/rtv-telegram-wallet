...
interface Store {
  ...
  setSubscription: (sub: 'free' | 'basic' | 'pro' | 'enterprise') => void;
}
...
export const useStore = create<Store>((set, get) => ({
  ...
  deductStars: (amount) => set((s) => ({ stars_balance: Math.max(0, s.stars_balance - amount) })),
  addStars: (amount) => set((s) => ({ stars_balance: s.stars_balance + amount })),
